  ...
  return (
    <div className="bg-gradient-to-br from-amber-500/5 via-neutral-900/90 to-neutral-950 border border-amber-500/30 rounded-3xl p-6 shadow-xl space-y-4 relative overflow-hidden text-white">
      ...
      <div className="bg-black/40 border border-amber-500/20 rounded-2xl p-4 space-y-2 relative z-10">
        ...
        {/* Private Feedback attached images */}
        {feedback.images && feedback.images.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 pt-2">
            {feedback.images.map((img, idx) => (
              <div 
                key={idx} 
                className="relative aspect-video rounded-2xl border border-amber-500/30 overflow-hidden bg-neutral-900/60 shadow-sm cursor-pointer hover:opacity-90 transition-opacity"
                onClick={() => setActiveImage(img)}
              >
                <img 
                  src={img} 
                  alt={`Confidential Attached ${idx + 1}`} 
      ...
      {activeImage && (
        ...
        >
          ...
          <img 
            src={activeImage} 
            alt="Confidential attached preview Large" 
