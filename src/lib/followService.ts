...
export async function checkIsFollowing(followerId: string, creatorId: string): Promise<boolean> {
  ...
  try {
    ...
    if (errStr.toLowerCase().includes('quota') || errStr.toLowerCase().includes('limit') || errStr.toLowerCase().includes('exceeded') || errStr.toLowerCase().includes('permission-denied')) {
      const cacheKey = `local_follow_${followerId}_${creatorId}`;
...
export async function toggleFollow(
  ...
  try {
    ...
    if (errStr.toLowerCase().includes('quota') || errStr.toLowerCase().includes('limit') || errStr.toLowerCase().includes('exceeded') || errStr.toLowerCase().includes('permission-denied')) {
      const cacheKey = `local_follow_${followerId}_${creatorId}`;
      const currentlyFollowing = localStorage.getItem(cacheKey) === 'true';
      const nextFollowing = !currentlyFollowing;
      localStorage.setItem(cacheKey, nextFollowing ? 'true' : 'false');
      
      const countKey = `local_follower_count_${creatorId}`;
      const currentCount = parseInt(localStorage.getItem(countKey) || '145', 10);
      ...
      return {
        ...
        message: nextFollowing ? "Đã theo dõi Creator! (Lưu cục bộ tạm thời do hạn ngạch cơ sở dữ liệu miễn phí đã hết)" : "Đã hủy theo dõi Creator. (Lưu cục bộ tạm thời)"
      };
    }
    
    // Fallback reconciliation if transaction hits edge case
    const currentActualCount = await reconcileFollowerCount(creatorId).catch(() => 0);
    const currentlyFollowing = await checkIsFollowing(followerId, creatorId).catch(() => false);
