  ...
  const filteredCharacters = characters
    ...
    .sort((a, b) => {
      ...
      if (sortOption === 'NEWEST') {
        const dateA = getTimeMs(a.createdAt);
        const dateB = getTimeMs(b.createdAt);
        return dateB - dateA;
      }
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
          {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
            <div key={i} className="aspect-[3/4] bg-neutral-100 dark:bg-neutral-800 rounded-2xl animate-pulse"></div>
          ))}
        </div>
