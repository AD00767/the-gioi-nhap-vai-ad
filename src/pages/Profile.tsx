...
export default function Profile() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
              ...
              <div>
                ...
                {user.role !== 'ADMIN' && user.role !== 'MODERATOR' && (
                  <div className="mt-1 flex justify-center md:justify-start">
                    <DisplayId type={user.creatorStatus ? 'creator' : 'user'} numericId={user.numericId} fallbackId={user.id} />
                  </div>
                )}
                {user.statusMessage && (
                  <div className="mt-2 flex justify-center md:justify-start">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 dark:bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-full text-xs font-semibold shadow-sm">
            ...
            {user.socialLinks && (
              ...
              (user.socialLinks.customLinks && user.socialLinks.customLinks.length > 0)
            ) && (
              <div className="flex items-center gap-3 mt-4 justify-center md:justify-start flex-wrap text-neutral-500">
                {user.socialLinks.facebook && (
                  <a href={user.socialLinks.facebook.startsWith('http') ? user.socialLinks.facebook : `https://${user.socialLinks.facebook}`} target="_blank" rel="noreferrer" className="hover:text-blue-600 transition-colors" title="Facebook">
                    <Facebook className="w-4 h-4" />
                  </a>
                )}
                {user.socialLinks.instagram && (
                  <a href={user.socialLinks.instagram.startsWith('http') ? user.socialLinks.instagram : `https://${user.socialLinks.instagram}`} target="_blank" rel="noreferrer" className="hover:text-pink-500 transition-colors" title="Instagram">
                    <Instagram className="w-4 h-4" />
                  </a>
                )}
                {user.socialLinks.tiktok && (
                  <a href={user.socialLinks.tiktok.startsWith('http') ? user.socialLinks.tiktok : `https://${user.socialLinks.tiktok}`} target="_blank" rel="noreferrer" className="hover:text-black dark:hover:text-white transition-colors" title="TikTok">
                    <Music className="w-4 h-4" />
                  </a>
                )}
                {user.socialLinks.discord && (
                  <span className="flex items-center gap-1 text-xs hover:text-indigo-500 transition-colors" title={`Discord: ${user.socialLinks.discord}`}>
                    <MessageSquare className="w-4 h-4" />
                    <span>{user.socialLinks.discord}</span>
                  </span>
                )}
                {user.socialLinks.customLinks && user.socialLinks.customLinks.map((link: any, idx: number) => {
                  ...
                  return (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold rounded-xl bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 transition-all border border-neutral-200/40 dark:border-neutral-700/60"
                      title={link.label}
                    >
            ...
            {/* Stats Bar */}
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-6 mt-8 pt-8 border-t border-neutral-100 dark:border-neutral-800 text-center md:text-left">
              <button onClick={() => setActiveTab('liked')} className="group flex flex-col items-center md:items-start hover:opacity-80 transition-all">
                <div className="text-2xl font-black tracking-tighter group-hover:text-amber-500 transition-colors">{totalLikesCount}</div>
                <div className="text-[10px] text-neutral-400 font-black uppercase tracking-[0.2em] mt-1">Lượt thích</div>
              </button>
              <button onClick={() => setActiveTab('bookmarks')} className="group flex flex-col items-center md:items-start hover:opacity-80 transition-all">
                <div className="text-2xl font-black tracking-tighter group-hover:text-amber-500 transition-colors">{totalCharSavesCount}</div>
                <div className="text-[10px] text-neutral-400 font-black uppercase tracking-[0.2em] mt-1">Lưu Character</div>
              </button>
              <button onClick={() => setActiveTab('bookmarks')} className="group flex flex-col items-center md:items-start hover:opacity-80 transition-all">
                <div className="text-2xl font-black tracking-tighter group-hover:text-amber-500 transition-colors">{totalPromptSavesCount}</div>
                <div className="text-[10px] text-neutral-400 font-black uppercase tracking-[0.2em] mt-1">Lưu Prompt</div>
              </button>
              <button onClick={handleOpenFollowersModal} className="group flex flex-col items-center md:items-start hover:opacity-80 transition-all">
                <div className="text-2xl font-black tracking-tighter group-hover:text-amber-500 transition-colors">{followersCount}</div>
                <div className="text-[10px] text-neutral-400 font-black uppercase tracking-[0.2em] mt-1">Người theo dõi</div>
              </button>
              <button onClick={handleOpenFollowingModal} className="group flex flex-col items-center md:items-start hover:opacity-80 transition-all">
                <div className="text-2xl font-black tracking-tighter group-hover:text-amber-500 transition-colors">{followingCount}</div>
                <div className="text-[10px] text-neutral-400 font-black uppercase tracking-[0.2em] mt-1">Đang theo dõi</div>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Tabs Navigation */}
      <div className="flex border-b border-neutral-200 dark:border-neutral-800 mb-8 overflow-x-auto scrollbar-none gap-2">
        <button
          onClick={() => setActiveTab('characters')}
          className={`flex items-center gap-2 py-4 px-6 text-[11px] font-black uppercase tracking-widest border-b-2 whitespace-nowrap transition-all ${
        ...
        >
          ...
          Yêu thích ({likedCharacters.length})
        </button>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                    {pinnedCharacters.map(char => (
                      <div key={char.id} className="group relative bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                        <div className="flex flex-col h-full">
                          {/* Image Section */}
                          <div className="relative aspect-[16/10] overflow-hidden bg-neutral-100 dark:bg-neutral-800 shrink-0">
                            <img 
                              src={char.avatar} 
                              alt={char.name} 
                            ...
                            <div className="absolute top-4 left-4">
                              <div className="bg-amber-500 text-white p-2 rounded-2xl shadow-xl animate-pulse">
                          ...
                          <div className="flex-1 p-6 flex flex-col min-w-0">
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <Link to={`/character/${char.id}`} className="text-xl font-black truncate hover:text-amber-500 transition-colors uppercase tracking-tight">
                                {char.name}
                              </Link>
                              <DisplayId type="character" numericId={char.numericId} fallbackId={char.id} className="bg-neutral-100 dark:bg-neutral-800 border-none text-neutral-400 text-[10px] font-black" />
                            </div>
                            
                            <p className="text-[13px] text-neutral-500 dark:text-neutral-400 line-clamp-2 mb-6 flex-1 font-medium italic">
                              "{char.slogan}"
                            </p>

                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-100 dark:border-neutral-800">
                              <div className="flex items-center gap-4 text-[10px] text-neutral-400 font-black uppercase tracking-widest">
                                <span className="flex items-center gap-1.5"><Heart className="w-3.5 h-3.5" /> {char.likesCount || 0}</span>
                                <span className="flex items-center gap-1.5"><Bookmark className="w-3.5 h-3.5" /> {char.savesCount || 0}</span>
                              </div>
                              
                              <div className="flex items-center gap-2">
                                <button 
                                  onClick={() => handleTogglePinCharacter(char)} 
                                  className="p-2.5 text-amber-500 hover:bg-amber-500/10 rounded-2xl transition-all"
                                  title="Bỏ ghim"
                                >
                                ...
                                <button 
                                  ...
                                  className="p-2.5 text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-2xl transition-all"
                                  title="Chỉnh sửa"
                                >
                                  <Edit3 className="w-4 h-4" />
                                </button>
                                <button 
                                  ...
                                  className="p-2.5 text-neutral-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all"
                                  title="Xóa"
                                >
                      ...
                      <div key={char.id} className="group relative bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                        <div className="flex flex-col h-full">
                          ...
                          <div className="relative aspect-[16/10] overflow-hidden bg-neutral-100 dark:bg-neutral-800 shrink-0">
                            ...
                            {char.pinned && (
                              <div className="absolute top-4 left-4">
                                <div className="bg-amber-500 text-white p-2 rounded-2xl shadow-xl">
                                  <Pin className="w-3.5 h-3.5 fill-current" />
                                </div>
                              </div>
                            )}
                          ...
                          <div className="flex-1 p-6 flex flex-col min-w-0">
                            ...
                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-100 dark:border-neutral-800">
                              <div className="flex items-center gap-4 text-[10px] text-neutral-400 font-black uppercase tracking-widest">
                                <span className="flex items-center gap-1.5" title="Lượt xem"><Eye className="w-3.5 h-3.5" /> {char.viewsCount || 0}</span>
                                <span className="flex items-center gap-1.5" title="Lượt thích"><Heart className="w-3.5 h-3.5" /> {char.likesCount || 0}</span>
                                <span className="flex items-center gap-1.5" title="Lượt lưu"><Bookmark className="w-3.5 h-3.5" /> {char.savesCount || 0}</span>
                              </div>
                              
                              <div className="flex items-center gap-2">
                                <button 
                                  onClick={() => handleTogglePinCharacter(char)} 
                                  className={`p-2.5 rounded-2xl transition-all ${char.pinned ? 'text-amber-500 bg-amber-500/10' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800'}`}
                                  title={char.pinned ? "Bỏ ghim" : "Ghim"}
                                >
                                  <Pin className={`w-4 h-4 ${char.pinned ? 'fill-current' : ''}`} />
                                </button>
                                <button 
                                  onClick={() => navigate(`/edit-character/${char.id}`)} 
                                  className="p-2.5 text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-2xl transition-all"
                  ...
                  {likedCharacters.map(char => (
                    <div key={char.id} className="group relative bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                      <div className="flex flex-col h-full">
                        ...
                        <div className="flex-1 p-6 flex flex-col min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <Link to={`/character/${char.id}`} className="text-xl font-black truncate hover:text-red-500 transition-colors uppercase tracking-tight">
                              {char.name}
                            </Link>
                            <DisplayId type="character" numericId={char.numericId} fallbackId={char.id} className="bg-neutral-100 dark:bg-neutral-800 border-none text-neutral-400 text-[10px] font-black" />
                          </div>
                          
                          <p className="text-[11px] text-neutral-500 dark:text-neutral-400 line-clamp-1 mb-2 italic font-bold">
                            Tác giả: {char.creatorName}
                          </p>
                          
                          <p className="text-[13px] text-neutral-400 line-clamp-2 mb-6 flex-1 font-medium italic">
                            "{char.slogan}"
                          </p>
                          ...
                          <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-100 dark:border-neutral-800">
                            <div className="flex items-center gap-4 text-[10px] text-neutral-400 font-black uppercase tracking-widest">
                              <span className="flex items-center gap-1.5" title="Lượt xem"><Eye className="w-3.5 h-3.5" /> {char.viewsCount || 0}</span>
                              <span className="flex items-center gap-1.5 text-red-500" title="Lượt thích"><Heart className="w-3.5 h-3.5 fill-current" /> {char.likesCount || 0}</span>
                              <span className="flex items-center gap-1.5" title="Lượt lưu"><Bookmark className="w-3.5 h-3.5" /> {char.savesCount || 0}</span>
                            </div>
                        ...
                        <div className="flex-1 p-6 flex flex-col min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-100 dark:border-neutral-800">
                            <div className="flex items-center gap-4 text-[10px] text-neutral-400 font-black uppercase tracking-widest">
                              <span className="flex items-center gap-1.5" title="Lượt xem"><Eye className="w-3.5 h-3.5" /> {char.viewsCount || 0}</span>
                              <span className="flex items-center gap-1.5" title="Lượt thích"><Heart className="w-3.5 h-3.5" /> {char.likesCount || 0}</span>
                              <span className="flex items-center gap-1.5 text-amber-500" title="Lượt lưu"><Bookmark className="w-3.5 h-3.5 fill-current" /> {char.savesCount || 0}</span>
                            </div>
