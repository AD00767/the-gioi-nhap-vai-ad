  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 animate-fade-in">
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-neutral-100 dark:border-neutral-800">
          <div className="space-y-2">
            ...
            <div className="mt-2">
              <DisplayId type="prompt" numericId={prompt.numericId} fallbackId={prompt.id} />
            </div>
        ...
        {prompt.link && (
          <div>
            ...
            <a 
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline bg-indigo-50 dark:bg-indigo-950/30 px-3 py-1.5 rounded-xl border border-indigo-100 dark:border-indigo-900/50"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              <span className="truncate max-w-xs md:max-w-md">{prompt.link}</span>
            </a>
          </div>
        )}
        ...
        {/* Ghi chú */}
        {prompt.notes && (
          <div className="p-5 bg-neutral-50 dark:bg-neutral-800/40 rounded-2xl border border-neutral-200/60 dark:border-neutral-800 space-y-2">
            <h3 className="text-xs font-extrabold uppercase tracking-wider text-neutral-400 flex items-center gap-1.5">
              <AlertCircle className="w-4 h-4 text-amber-500" />
              <span>Ghi chú từ tác giả</span>
            </h3>
        ...
        {prompt.images && prompt.images.length > 0 && (
          <div>
            <h3 className="text-xs font-extrabold uppercase tracking-wider text-neutral-400 mb-3">
              Ảnh minh họa / Kết quả mẫu ({prompt.images.length})
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {prompt.images.map((img, idx) => (
                <div 
                  key={idx} 
                  className="relative aspect-video rounded-2xl border border-neutral-200/80 dark:border-neutral-800 overflow-hidden bg-neutral-100 dark:bg-neutral-900 shadow-sm cursor-pointer hover:opacity-90 transition-opacity"
                  onClick={() => setActiveImage(img)}
                >
                  <img 
                    src={img} 
                    alt={`Prompt Result Capture ${idx + 1}`} 
      ...
      {activeImage && (
        ...
        >
          <button 
            className="absolute top-4 right-4 p-3 rounded-full bg-neutral-800/80 hover:bg-neutral-700/80 text-white transition-colors cursor-pointer"
            onClick={() => setActiveImage(null)}
            title="Đóng"
          >
            <X className="w-6 h-6" />
          ...
          <img 
            src={activeImage} 
            alt="Result Preview Large" 
            className="max-w-full max-h-[95vh] object-contain rounded-xl select-none"
            referrerPolicy="no-referrer"
